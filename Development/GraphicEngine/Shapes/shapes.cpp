...
namespace shapes {
    ...
    Line::Line(glm::vec3 const& _start, glm::vec3 const& _dir) :
        start{ _start },
        dir{ _dir }
    {}

    glm::vec3 Line::evaluate(float t) const {
        return start + t * dir;
    }

    Aabb::Aabb(glm::vec3 const& min_point, const glm::vec3& max_point) :
    ...
    Aabb::Aabb(const glm::vec3* points, size_t count):
        min{*points },
        max{*points }
    {
    ...
    Aabb::Aabb(const glm::vec3* points, size_t count, const glm::mat4& transformMatrix) :
        Aabb(points, count)
    {
    ...
    Aabb::Aabb(Aabb const& bv, glm::mat4 const& _transform) :
        min{ bv.min },
        max{ bv.max }
    {
    ...
    Aabb::Aabb(glm::vec3 const& center, float width, float height, float depth) {
        glm::vec3 halfExtents(width * 0.5f, height * 0.5f, depth * 0.5f);

        min = center - halfExtents;
        max = center + halfExtents;
    }

    Aabb Aabb::transform( glm::mat4 const& m2w) const {
        ...
        glm::vec3 point = glm::vec3{ m2w * glm::vec4{corners[0], 1.f} };
        Aabb aabb(point,point);

        // transform all corners and get the min and max
        for (size_t n{1}; n < corners.size(); n++) {

            glm::vec3 transformedPoint = glm::vec3{ m2w * glm::vec4{corners[n], 1.f}};
    ...
    bool Aabb::intersects( glm::vec3 const& pt) const {
    ...
    glm::vec3 Triangle::closest(glm::vec3 const& pt) const {
        glm::vec3 p1pt = pt - points[1];
        glm::vec3 triangleNormal = normal();
        ...
        glm::vec3 ptPtPrime = -(lengthPtPtprime) * (triangleNormal / normalLength);
    ...
    glm::vec3 const& Triangle::operator[](int index) const {
        return points[index];
    }
    ...
    bool Sphere::contains(glm::vec3 const& pt) const {
        // Calcualte distance between point and sphere center

        glm::vec3 line = center - pt;
    ...
    bool Sphere::intersects(Sphere const& rhs) const {
        ...
        glm::vec3 line = center - rhs.center;
        double sqrDistance = line.x * line.x + line.y * line.y + line.z * line.z;
    ...
    Sphere Sphere::centroid( glm::vec3 const* points, size_t count){
        glm::vec3 sumOfPostions{};

        for (size_t n{}; n < count; n++) {
            sumOfPostions += points[n];
        }

        glm::vec3 averagePoint = sumOfPostions / static_cast<float>(count);
    ...
    Sphere Sphere::centroid( glm::vec3 const* points, size_t count, const glm::mat4& transformMatrix) {
        
        std::vector<glm::vec3> transformedPoints;
        transformedPoints.reserve(count);
        for (size_t n{}; n < count; n++) {
            transformedPoints.emplace_back(transformMatrix * glm::vec4{ points[n], 1.f });
        }
    ...
    Sphere Sphere::ritters( glm::vec3 const* points, size_t count) {

        //first for min point, second for max point
        std::pair<glm::vec3, glm::vec3> xLine{ points[0], points[0]};
        std::pair<glm::vec3, glm::vec3> yLine{ points[0], points[0] };
        std::pair<glm::vec3, glm::vec3> zLine{ points[0], points[0] };

        for (size_t n{}; n < count ; n++) {

            const glm::vec3& position = points[n];
    ...
    Sphere Sphere::ritters( glm::vec3 const* points, size_t count, const glm::mat4& transformMatrix) {
    ...
    Sphere Sphere::iterative( glm::vec3 const* points, size_t count, size_t iteration_count, float shrink_ratio) {
    ...
    Sphere Sphere::iterative( glm::vec3 const* points, size_t count, size_t iteration_count, float shrink_ratio, const glm::mat4& transformMatrix) {
        std::vector<glm::vec3> transformedPoints;
        transformedPoints.reserve(count);
        for (size_t n{}; n < count; n++) {
            transformedPoints.emplace_back(transformMatrix * glm::vec4{ points[n], 1.f });
    ...
    Segment::Segment( glm::vec3 const& start,  glm::vec3 const& end) :
        points{ start, end }
    {}

    glm::vec3 Segment::closest(Segment const& rhs) const {


        glm::vec3 v = points[1] - points[0];
        glm::vec3 w = rhs[1] - rhs[0];
        glm::vec3 k = points[0] - rhs[0];
        ...
        glm::vec3 closestPoint1 = points[0] + t * v;

        return closestPoint1;
    }

    float Segment::distance(glm::vec3 const& pt) const {
        //Get line from segment start point to pointM
        glm::vec3 segmentPointM = pt - points[0];
        glm::vec3 segmentDir = dir();
        ...
        // calculate closest point of the segment
        glm::vec3 closestPoint = points[0] + t * segmentDir;

        return glm::length(pt - closestPoint);

    }

    glm::vec3 Segment::at(float t) const {
        return points[0] + dir() * t;
    }

    glm::vec3 Segment::dir() const {
        return points[1] - points[0];
    }

    glm::vec3 const& Segment::operator[](int index) const {
        return points[index];
    }

    glm::vec3& Segment::operator[](int index) {
        return points[index];
    }
    ...
    Plane::Plane( glm::vec3 const& _normal, float d) :
        normal{_normal},
        d{ d }
    {}

    Plane::Plane( glm::vec3 const& point,  glm::vec3 const& _normal):
        normal{_normal},
        d{glm::dot(_normal, point)}
    {}

    float Plane::distance(glm::vec3 const& pt) const {
        return glm::dot(normal, pt) - d;
    }

    glm::vec3 Plane::closest_point( glm::vec3 const& pt) const {
        glm::vec3 segmentPointM = pt - (d * normal);
        float shortestDistance = glm::dot(segmentPointM, normal);
        return pt - (normal * shortestDistance);

    }

    SideResult Plane::classify( glm::vec3 const& pt) const {

        float distance = glm::dot(normal, pt) - d;
    ...
    SideResult Plane::classify(const Aabb& aabb)const {
        ...
        glm::vec3 aabbCenter = (aabb.max + aabb.min) * 0.5f;
    ...
    Frustum::Frustum(std::array<glm::vec3, 6> const& normals, std::array<float, 6> const& dists):
    ...
    Frustum::Frustum(glm::mat4 const& viewProj):
        planes{ Plane(glm::vec3(viewProj[0][3] + viewProj[0][0], viewProj[1][3] + viewProj[1][0], //Left
                            viewProj[2][3] + viewProj[2][0]),viewProj[3][3] + viewProj[3][0]),
                 Plane(glm::vec3(viewProj[0][3] - viewProj[0][0], viewProj[1][3] - viewProj[1][0], //Right
                            viewProj[2][3] - viewProj[2][0]),viewProj[3][3] - viewProj[3][0]),
                 Plane(glm::vec3(viewProj[0][3] + viewProj[0][1], viewProj[1][3] + viewProj[1][1], //Bottom
                            viewProj[2][3] + viewProj[2][1]),viewProj[3][3] + viewProj[3][1]),
                 Plane(glm::vec3(viewProj[0][3] - viewProj[0][1], viewProj[1][3] - viewProj[1][1], //Top
                            viewProj[2][3] - viewProj[2][1]),viewProj[3][3] - viewProj[3][1]),
                 Plane(glm::vec3(viewProj[0][3] + viewProj[0][2], viewProj[1][3] + viewProj[1][2],//Near 
                            viewProj[2][3] + viewProj[2][2]),viewProj[3][3] + viewProj[3][2]), 
                 Plane(glm::vec3(viewProj[0][3] - viewProj[0][2], viewProj[1][3] - viewProj[1][2], //Far
                            viewProj[2][3] - viewProj[2][2]),viewProj[3][3] - viewProj[3][2])}
    {
    ...
    Ray::Ray(glm::vec3 const& _start, glm::vec3 const& _dir) :
        start{ _start },
        dir{ _dir }
     {}
    ...
    float Ray::intersect(Plane const& plane) const{
    ...
    float Ray::intersect(Sphere const& sphere) const {
    ...
    float Ray::intersect(Triangle const& triangle) const {
        
        // test intersection of triangle plane
        glm::vec3 triangleNormal = glm::cross(triangle[1] - triangle[0], triangle[2] - triangle[0]);
        Plane trianglePlane(triangle[0], triangleNormal);

        float planeIntersection = intersect(trianglePlane);

        // If ray does not intersect triangle plane, return -1
        if (planeIntersection < cEpsilon) {
            return -1.f;
        }

        //find intersection point
        glm::vec3 intersectionPoint = start + planeIntersection * dir;

        //Calcualte vector for 2 lines on the triangle
        glm::vec3 v = triangle[1] - triangle[0];
        glm::vec3 w = triangle[2] - triangle[0];

        //Calculate vector between intersection point and point on triangle
        glm::vec3 u = intersectionPoint - triangle[0];

        //Calculate paremeter for equations
        float a = glm::dot(v, v);
        float b = glm::dot(w, v);
        float c = glm::dot(v, w);
        float d = glm::dot(w, w);
        ...
        // Apply Cramer's rule
        float denom = glm::determinant(glm::mat2x2(a, c, b, d));

        // Ray start point is on the triangle
        if (denom < cEpsilon) {
            return 0.f;
        }

        float s = glm::determinant(glm::mat2x2(e, f, b, d)) / denom;
        float t = glm::determinant(glm::mat2x2(a, c, e, f)) / denom;
